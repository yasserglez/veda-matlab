function [population, evaluation] = ...
    replacing_none(params, current_population, current_evaluation, ...
                   previous_population, previous_evaluation)
  % A replacing method that does not modify the population.
  %
  % PARAMS is a struct with the parameters of the EDA. CURRENT_POPULATION should
  % be the population sampled in the current generation and CURRENT_EVALUATION
  % the valuation of each solution of this population. PREVIOUS_POPULATION
  % should be the population of the previous generation after the replacing
  % method was applied and PREVIOUS_EVALUATION the evaluation of each solution
  % of this population.
  %
  % The output variable POPULATION is the population that the algorithm should
  % use for this generation and EVALUATION the evaluation of each solution in
  % this population.

  population = current_population;
  evaluation = current_evaluation;
end
